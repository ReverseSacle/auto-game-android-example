...
from game_event.normal_event import check_dialog, check_return_city, check_if_in_city, check_if_loading
...
def monster_select_5(threadshold=0.6):
    monster_area_path = save_image_monster_path + '5.png'
...
def process(time_wait=2):
    print("first chaperter - six quest start.....")
    check_if_quest_entering()
    check_dialog()

    while False == check_if_battling(): monster_select_5()
    ...
    time.sleep(5)
    print("first chaperter - six quest start end")
    check_if_in_city()
    check_if_loading()
    city_info_clean()